...
SX1302_RESET_PIN=120     # SX1302 reset
SX1302_POWER_EN_PIN=129  # SX1302 power enable
...
#    echo "$AD5338R_RESET_PIN" > /sys/class/gpio/export; WAIT_GPIO
    echo "$SX126X_NSS_PIN" > /sys/class/gpio/export; WAIT_GPIO
...
#    echo "out" > /sys/class/gpio/gpio$AD5338R_RESET_PIN/direction; WAIT_GPIO
    echo "out" > /sys/class/gpio/gpio$SX126X_NSS_PIN/direction; WAIT_GPIO
}
...
#    echo "CoreCell ADC reset through GPIO$AD5338R_RESET_PIN..."
    ...
    echo "1" > /sys/class/gpio/gpio$SX126X_NSS_PIN/value; WAIT_GPIO
...
term() {
    ...
    if [ -d /sys/class/gpio/gpio$SX126X_NSS_PIN ]
    then
        echo "$SX126X_NSS_PIN" > /sys/class/gpio/unexport; WAIT_GPIO
    fi
}
